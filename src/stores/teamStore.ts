...
export const useActiveTeams = () => {
	...
	return React.useMemo(() => 
		teams.filter(team => team.status === ETeamStatus.ACTIVE),
		[teams]
	);
};
...
export const useTeamsByEntity = (entity: string) => {
	const teams = useTeamStore(state => state.teams);
	return React.useMemo(() => 
		teams.filter(team => team.entity === entity),
		[teams, entity]
	);
};
...
export const useTeamsByManager = (manager: string) => {
	const teams = useTeamStore(state => state.teams);
	return React.useMemo(() => 
		teams.filter(team => team.manager === manager),
		[teams, manager]
	);
};

// Search and filter utilities
export const useFilteredTeams = (searchTerm: string, statusFilter: string, entityFilter: string) => {
	const teams = useTeamStore(state => state.teams);
